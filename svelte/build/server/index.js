import { c as create_ssr_component, s as setContext, v as validate_component, m as missing_component, n as noop, a as safe_not_equal } from './chunks/index-af1a7b44.js';
...
function get_cookies(request, url, options) {
  ...
  const initial_cookies = parse_1(header);
  const new_cookies = {};
  const defaults = {
    httpOnly: true,
    sameSite: "lax",
  ...
  const cookies = {
    get(name, opts) {
      ...
      const req_cookies = parse_1(header, { decode });
      const cookie = req_cookies[name];
      if (!options.dev || cookie) {
        return cookie;
      }
      if (c || ((_a = cookie_paths[name]) == null ? void 0 : _a.size) > 0) {
        console.warn(
